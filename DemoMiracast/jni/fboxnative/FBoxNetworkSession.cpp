//DEVELOP PRADEEP 23/03/2021

#define LOG_NDEBUG 0
#define LOG_TAG "FBoxNetworkSession"
#include <utils/Log.h>
#include <arpa/inet.h>
#include <fcntl.h>
#include <linux/tcp.h>
...
struct FBoxNetworkSession::NetworkThread : public Thread {
    explicit NetworkThread(FBoxNetworkSession *session);

protected:
    virtual ~NetworkThread();

private:
    FBoxNetworkSession *mSession;
...
struct FBoxNetworkSession::Session : public RefBase {
    enum Mode {
        MODE_RTSP,
        MODE_DATAGRAM,
        MODE_WEBSOCKET,
...
FBoxNetworkSession::NetworkThread::NetworkThread(FBoxNetworkSession *session)
    : mSession(session) {
}

FBoxNetworkSession::NetworkThread::~NetworkThread() {
}

bool FBoxNetworkSession::NetworkThread::threadLoop() {
    mSession->threadLoop();
...
FBoxNetworkSession::Session::Session(
        int32_t sessionID,
        State state,
        int s,
        bool isRTPSession,
...
int32_t FBoxNetworkSession::Session::sessionID() const {
    return mSessionID;
}

int FBoxNetworkSession::Session::socket() const {
    return mSocket;
}

void FBoxNetworkSession::Session::setMode(Mode mode) {
    mMode = mode;
}

status_t FBoxNetworkSession::Session::switchToWebSocketMode() {
...
sp<AMessage> FBoxNetworkSession::Session::getNotificationMessage() const {
    return mNotify;
}

bool FBoxNetworkSession::Session::isRTSPServer() const {
    return mState == LISTENING_RTSP;
}

bool FBoxNetworkSession::Session::isTCPDatagramServer() const {
    return mState == LISTENING_TCP_DGRAMS;
}

bool FBoxNetworkSession::Session::wantsToRead() {
    return !mSawReceiveFailure && mState != CONNECTING;
}

bool FBoxNetworkSession::Session::wantsToWrite() {
...
status_t FBoxNetworkSession::Session::readMore() {
    if (mState == DATAGRAM) {
...
status_t FBoxNetworkSession::Session::writeMore() {
    if (mState == DATAGRAM) {
...
status_t FBoxNetworkSession::Session::sendRequest(
        const void *data, ssize_t size, bool timeValid, int64_t timeUs) {
    ...
    if (mState == CONNECTED && mMode == MODE_DATAGRAM) {
        ...
        static const bool kUseMask = false; 
...
void FBoxNetworkSession::Session::notifyError(
...
void FBoxNetworkSession::Session::notify(NotificationReason reason) {
    sp<AMessage> msg = mNotify->dup();
    msg->setInt32("sessionID", mSessionID);
...
FBoxNetworkSession::FBoxNetworkSession()
    : mNextSessionID(1),
...
status_t FBoxNetworkSession::start() {
    if (mThread != NULL) {
        return INVALID_OPERATION;
    }
    ...
    status_t err = mThread->run("FBoxNetworkSession", ANDROID_PRIORITY_AUDIO);
...
status_t FBoxNetworkSession::stop() {
    if (mThread == NULL) {
        return INVALID_OPERATION;
    }
...
status_t FBoxNetworkSession::createRTSPClient(
...
status_t FBoxNetworkSession::createRTSPServer(
...
status_t FBoxNetworkSession::createUDPSession(
...
status_t FBoxNetworkSession::createTCPDatagramSession(
        const struct in_addr &addr, unsigned port,
        const sp<AMessage> &notify, int32_t *sessionID) {
    return createClientOrServer(
...
status_t FBoxNetworkSession::destroySession(int32_t sessionID) {
...
status_t FBoxNetworkSession::MakeSocketNonBlocking(int s) {
...
void FBoxNetworkSession::setRTPConnectionState(bool state){
    mIsRTPConnection = state;
}
status_t FBoxNetworkSession::createClientOrServer(
        Mode mode,
...
status_t FBoxNetworkSession::connectUDPSession(
...
status_t FBoxNetworkSession::sendRequest(
...
status_t FBoxNetworkSession::switchToWebSocketMode(int32_t sessionID) {
    Mutex::Autolock autoLock(mLock);
...
void FBoxNetworkSession::interrupt() {
...
void FBoxNetworkSession::threadLoop() {
    fd_set rs, ws;
    FD_ZERO(&rs);
    FD_ZERO(&ws);
